...
//populates names + descs into 'entries' array
...
// add default miku to the
sidebarEntries.push(new entry (entries[23].number, entries[23].name, entries[23].desc,));
...
$('.term').click(function(){
  ...
  $('.sidebarEntries').append(
    '<div class="sidebarEntry ' + entries[posInArray].number + '">' +
  ...
  if ($('.sidebarLeft').hasClass('hideDisplay')) {
    ...
    $('.hidePlaylist').text('hide sidebar');
  ...
  sidebarEntries.push(new entry(entries[posInArray].number, entries[posInArray].name, entries[posInArray].desc));

  //remove empty array items
  sidebarEntries.filter(Boolean);
...
$('.sidebarLeft').on('click', '.sidebarEntry .entryX', function(){
  ...
  var entryNum = classList[1];
  console.log(entryNum);

  const findGuy = sidebarEntries.findIndex(
    element => element.number == entryNum
  );
  console.log(findGuy);

  sidebarEntries.splice(findGuy, 1);
  sidebarEntries.filter(Boolean);
...
for (let i = 0; i<entries.length; i++){
  ...
  $('<div></div>').appendTo($('.footerContent')).addClass('footerEntry ' + entries[i].number);
...
//click glossary item --> add to sidebar
$('.footerName').click(function(){
  ...
  var classList = $(this).parent('.footerEntry').attr('class').split(/\s+/);
  var glossNum = classList[1];
  var posInArray = classList[1] - 1;
...
//click glossary term appearance num --> jump to spot in text
$('.textNumMention').click(function(){
  const numAsString = $(this).text();
  var numAsVar = parseInt(numAsString);
  console.log(numAsVar);

  //search all .term for a match
  $('.term').each(function(){
    var classList = $(this).attr('class').split(/\s+/);
    var textNum = classList[2];
    ...
    if (textNum == numAsVar){
      $(this).closest('.page').siblings('.page').removeClass('hideDisplay').addClass('hideDisplay');
      $(this).closest('.page').removeClass('hideDisplay');

      citPos();


      var posOfTerm = $(this).position().top;
      console.log(posOfTerm);

      $(this).closest('.page').scrollTop(posOfTerm - (window.innerHeight/2 - 200));


      console.log($(this).closest('.page').scrollTop());

      //bunch of if statements to hide/show the right section
      //jump to the scroll pos of the element
      //YAY IT worked
    }
  });
});
...
$('.hidePlaylist').click(function () {
  ...
  if (playlistCounter == 0) {
    //playlist is hidden
    $('.hidePlaylist').text('show sidebar');
...
//   alert("Oh no! Safari detected! This site will not work on Safari. Try again on Firefox (preferred) or Chrome. -Christine");
//   $("html").remove();
// }
if (navigator.userAgent.search("Safari") >= 0 && navigator.userAgent.search("Chrome") < 0) {
  alert("Oops, this site does not currently work on Safari. Try again on Firefox (preferred) or Chrome. -CW");
  $("html").remove();
}
...
if (/Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
    $("html").remove();
    alert("Oops, this site does not have a mobile version yet. Try again on desktop. -CW");
} else {
}
